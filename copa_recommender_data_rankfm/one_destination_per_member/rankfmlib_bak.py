...
def topn_recommendations(model, interactions_dct, base_rec, topN=3):
    ...
    k = topN
    most_popular = interactions_train.groupby('product_id')['user_id'].count().sort_values(ascending=False)[:topN]
...
#----------------------------------------------------------------------------------------
def calculate_dct_with_attributes(nb_samples=10000): 
    train_year = 2016
    valid_year  = 2017
    years = [train_year, valid_year]

    #df = read_data(train_year, valid_year)
    member_dest_df, df_item_onehot = read_data_attributes(2016, train_year, valid_year)
    #print(df_item_onehot[train_year].head())
